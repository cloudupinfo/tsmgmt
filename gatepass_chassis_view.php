...
if(!empty($veihicle))
{
	// Customer Detail Add then gatpass generate code
	$customer = $db->getRow("SELECT * FROM ".$table1." where `product_id`=?",array($product['product_id']));
	if(!empty($customer))
	{
		$select = $db->getRow("SELECT * FROM ".$table2." where `product_id`=?",array($product['product_id']));$product_price = $db->getRow("SELECT * FROM ".$table3." where `product_id`=?",array($product['product_id']));
		// find salesman name
	...
	$_SESSION['admin_error'] = "Please Select Model Then GatePass Generate Contact Sub-Admin Or Admin...";
	header('Location:gatepass.php');
	exit();
}
	...
	<div class="main-content">
		...
		<div class="container">
			...
			<div class="row">
				...
				<div class="col-sm-12">
					<div class="page-header">
						<h3>Generate Gate Pass <?php echo $type;?></h3>
					</div>
					<form class="form-horizontal" method="post" action="model/gatepassManage.php" id="gatepass_add_frm" name="gatepass_add_frm" enctype="multipart/form-data" target="_blank">
						...
						<input type="hidden" value="<?php echo !empty($gateId) ? $gateId : '0';?>" name="id">
						<div class="form-group">
							...
							<div class="col-sm-7">
								<?php if($type=="add"){?>
									<input type="submit" class="btn btn-success btn-squared" value="Print">
								<?php }else{ ?>
									<input type="submit" class="btn btn-info btn-squared" value="Duplicate Print">
								<?php }?>
								<a href="gatepass_list.php" class="btn btn-danger btn-squared">Cancel</a>
							</div>
						</div>
					</form>
				</div>
