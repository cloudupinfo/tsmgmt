...
$gatepass = $db->getRow("SELECT * FROM ".$table." where `gatepass_id`=?",array($id));
if(!empty($gatepass['gatepass_id']) && ($gatepass['gatepass_id']!=0)){
	$product = $db->getRow("SELECT * FROM ".$table1." where `product_id`=?",array($gatepass['product_id']));
}
if(!empty($product))
{
