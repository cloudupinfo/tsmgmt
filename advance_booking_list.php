			...
			<div class="row">
				<div class="col-sm-12">
					...
					<div class="page-header">
						<h1>Advance Booking List</h1>
					</div>
					<div id="nestable-menu" class="pull-right">
						<a href="advance_booking.php" class="btn btn-primary btn-squared">Advance Booking Add</a>
					</div>
...
$(document).ready(function() {
	...
	table = $('#loadmaincat').DataTable({    
		"ajax" : {
			"url" : "model/advancebookingList.php",
			"dataSrc": "data",
			"type" : "POST",
			"data" : {
				"list" : "list",
	...
	$(document).on("click", ".to_delete", function(e) {
		...
		var a_delete = "advance_delete";
		select.addClass("selected");

		if (confirm('Are you sure you want to delete advance payment ?')) {
			$.ajax({
				type : "POST",
				url : "model/advancebookingManage.php",
				cache : false,
				data : {
	...
	$(document).on("click", ".refund", function(e) {
		var id = $(this).attr("id");
		var refund1 = $(this).attr("refund");
		var refund = "advance_refund";
		if(refund1==1)
			refund_1 = 0;
		else
			refund_1 = 1;
		...
		if (confirm('Are you sure you want to refund it?')) {
			$.ajax({
				...
				cache : false,
				data : {
					type : refund, mid : id , refund : refund_1
				},
				beforeSend: function() {
					$(".loading-div").removeClass('no-display');
					$(".loading-div").addClass('display');
				...
				success : function(result) {
					...
					$(".loading-div").addClass('no-display');
					if (result) {
						alert("Refund is Successfully");
						table.row('.selected').remove().draw(false);
						table.ajax.reload();
					}else {
						alert("Does not Refund it : \n" + result);
						select.removeClass('selected');
						table.ajax.reload();
					}
				}
